...
@Component
@Log4j2
public class LanApiLogAop {
    // 初始化的时候找到日志处理器，1.找不到就不再去管理日志 2.提供一个默认的日志处理器

    private final LanApiLogHandler lanApiLogHandler;

    public LanApiLogAop(@Autowired(required = false) LanApiLogHandler lanApiLogHandler) {
        this.lanApiLogHandler = lanApiLogHandler;
    }
    ...
    public void apiLog(JoinPoint joinPoint) {
        ...
        try {
            lanApiLogHandler.handle(apiName, currentRequestIp, path, paramsMap);
        } catch (Exception e) {
