...
public class SnowFlake {
    ...
    public SnowFlake(long datacenterId, long machineId, int seedLength) {
    ...
    public synchronized String nextId() {
        ...
        long id = (currTimestamp - START_TIMESTAMP) << TIMESTAMP_LEFT // 时间戳部分
                ...
                | sequence; // 序列号部分
        return id + buildRandomStr(seedLength);
    }

    /**
     * 生成指定长度的随机字符串
     */
    private static final char[] seed = "abcdefghijklmnopqrstuvwxyzZ0123456789".toCharArray();

    private static String buildRandomStr(int length) {
        StringBuilder sb = new StringBuilder(length);
        for (int i = 0; i < length; i++) {
            char ch = seed[(int) (Math.random() * seed.length)];
            sb.append(ch);
        }
        return sb.toString();
    }
